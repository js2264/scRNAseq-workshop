...
> Try and compute RNA velocity. What do you see? Does this help inferring direction of the trajectory? 
...
velo_out <- velociraptor::scvelo(
    testis, 
    assay.X = "counts", 
    subset.row = rowData(testis)$isHVG, 
    use.dimred = "corrected"
)
embedded_velo <- velociraptor::embedVelocity(reducedDim(testis, "TSNE"), velo_out)
grid.df <- velociraptor::gridVectors(reducedDim(testis, "TSNE"), embedded_velo, resolution = 30)
p <- scater::plotReducedDim(testis, 'TSNE', colour_by = "annotation", point_alpha = 0.5) +
    geom_segment(
        data = grid.df, 
